...
func (self *KiteMysqlStore) batchCommit(hashId int, messageId []string) bool {
	...
	err, stmt := p.Get()
	if nil != err {
		log.ErrorLog("kite_store", "KiteMysqlStore|batchCommit|GET STMT|FAIL|%s|%d\n", err, hashId)
		return false
	}
...
func (self *KiteMysqlStore) batchDelete(hashId int, messageId []string) bool {
	...
	err, stmt := p.Get()
	if nil != err {
		log.ErrorLog("kite_store", "KiteMysqlStore|batchDelete|GET STMT|FAIL|%s|%d\n", err, hashId)
		return false
	}
	defer p.Release(stmt)
...
func (self *KiteMysqlStore) batchUpdate(hashId int, entity []*MessageEntity) bool {
	...
	err, stmt := p.Get()
	if nil != err {
		log.ErrorLog("kite_store", "KiteMysqlStore|batchUpdate|GET STMT|FAIL|%s|%d\n", err, hashId)
		return false
	}
	defer p.Release(stmt)
